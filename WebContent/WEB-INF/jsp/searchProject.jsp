				...
				<table class="table table-striped table-hover">
					...
					<tbody>
						...
						<tr>
							...
							<td>${var.name}</td>
							<td>
							<fmt:formatDate value="${var.startDate}" pattern="yyyy/MM/dd" />
							</td>
							<td>
							<fmt:formatDate value="${var.endDate}" pattern="yyyy/MM/dd" />
							</td>
							<td>${var.budget}</td>
							<td>${var.content}</td>
							<td>${var.username}</td>
							...
							<c:choose>
   								<c:when test="${user.roleId==1&&var.state!=1}">  
    								<a href="<%=basePath%>deleteProject/${var.id}.html">删除</a>
    								
    								<a href="<%=basePath%>updateProject/${var.id}.html">编辑</a>
   								</c:when>
							</c:choose>
							
							<!-- 
							<c:choose>
   								<c:when test="${user.roleId==0&&var.state==0}">  
    								<a href="<%=basePath%>checkProject/${var.id}.html?action=1">通过</a>
    								<a href="<%=basePath%>checkProject/${var.id}.html?action=2">不通过</a>
   								</c:when>
							</c:choose>
